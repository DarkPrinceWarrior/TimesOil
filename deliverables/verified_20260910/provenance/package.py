...
for label, root, expected in [('model_y/baseline', Path(audit['baseline_run']), audit['baseline_chdd_m']),
    ...
    if label.endswith('/selected'):
        ...
        copy(R / 'timesfm-monthly-y-v5-20260909/full-audit.json', 'model_y/full-audit.json')

z_new = next(pair['candidate'] for pair in selection['comparisons'] if pair['candidate']['run'].endswith('/candidate-v2'))
