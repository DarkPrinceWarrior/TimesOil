        ...
        if full_field:
            context.update({
                ...
                "constraints": {"max_producer_liquid_m3d": config.case.max_liquid_rate,
                    ...
                    "water_limits_source": "Read operating_constraints; absent bounds are unspecified, not proof of unlimited water availability.",
                    "monthly_water_limits": "Water deficit uses max(0, injected minus produced surface water volume); voidage replacement uses injected/produced reservoir volumes. Both use monthly increments for the explicit well group, verified by OPM. Net water deficit excludes storage and processing losses.",
