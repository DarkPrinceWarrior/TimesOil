...
_ROLE_PROMPTS = {
    ...
    AgentRole.CRITIC: (
        "Ты критик. Сначала установи approval_scope: что именно требуется подтвердить сейчас. "
        "Для завершённого физического расчёта обязательны квитанции OPM, происхождение данных, "
        "проверка ограничений и официальный ЧДД за явно указанный период. Одобряй только "
        "при наличии этих доказательств; проверенные инструменты важнее предварительного текста ролей. "
        "Если суррогат заменяет полный OPM при выборе управления, обязательны подтверждённое "
        "обучение/валидация и UQ/OOD; без них отклони такой выбор. Если суррогат только предлагает "
        "гипотезы, а каждый сравниваемый кандидат рассчитан OPM за весь оставшийся период и выбран "
        "по официальному ЧДД, проверяй этот физический результат. Отдельно укажи ошибки, UQ/OOD "
        "и пределы применимости суррогата; это не является его сертификацией. "
        "Улучшение ЧДД подтверждай только при сопоставимой базе и одинаковом полном периоде. "
        "Когда улучшение не заявлено, отсутствие базы не отменяет корректность отдельного расчёта. "
        "Не сравнивай накопленный ЧДД одного месяца с ЧДД полного продолжения. "
        "Подтверждение завершённого месяца или численной пары не означает разрешение автономного "
        "развёртывания, сертификацию суррогата или конкурсный результат. Не приписывай запросу эти цели. "
        "is_baseline=false сохраняется после OPM. Одобрение не заменяет детерминированные защиты."
    ),
}
...
class AgentWorkflow:
    ...
    async def _run_role(self, role: AgentRole, state: AgentState) -> RoleDecision:
        ...
        messages = (
            ChatMessage("system", _ROLE_PROMPTS[role] + ("" if role is AgentRole.CRITIC else (
                " Сейчас выполняется численная проверка уже завершённой пары OPM/ЧДД. "
                "Поле approved подтверждает только корректность численного сравнения в заданном scope. "
                "Прочитай проверенные результаты инструмента: одинаковые исходная физика, история, "
                "полный период, ограничения и экономика обязательны. Отклони неподтверждённое сравнение. "
                "Новый запуск симулятора не запрашивается. Это не разрешение автономного управления "
                "и не сертификация суррогата, UQ/OOD или соответствия эталонному симулятору."
                if state.context.get("phase") == "completed_paired_numerical_audit" else
                " Сейчас фаза планирования до запуска симулятора. Поле approved означает разрешение "
                "следующего вычислительного шага, а не подтверждение его результата. Отсутствие будущих "
                "квитанций OPM/ЧДД само по себе не блокирует их вычисление. Отсутствие обучения суррогата "
                "не блокирует повторный расчёт исходного baseline без использования суррогата. "
                "Отклоняй запуск при нарушенных ограничениях или непроверенных обязательных входах; "
                "неизвестные показатели явно обозначай, не выдумывай их. Финальные квитанции оценивает критик."
            ))),
            ChatMessage(
                "user",
