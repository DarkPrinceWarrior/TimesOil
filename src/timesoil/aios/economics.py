...
def _management_months(period: tuple[date, date]) -> tuple[date, date]:
    if (
        not isinstance(period, tuple) or len(period) != 2
...
def opm_management_rows(
    ...
    for row in rows:
        stamp = date.fromisoformat(str(row["DATA"]))
        if stamp <= end:
            result.append({**row, "DATA": (stamp - timedelta(days=1)).replace(day=1).isoformat()})
    return result
...
def management_period_summary(
    ...
    inflow = math.fsum(_finite_result(row.get("discountedInflowM"), "discountedInflowM") for row in selected)
    outflow = math.fsum(_finite_result(row.get("discountedOutflowM"), "discountedOutflowM") for row in selected)
    return {
        "start_inclusive": start.isoformat(), "end_exclusive": end.isoformat(),
        "months": expected,
        "total_chdd_m": math.fsum(_finite_result(row.get("chddM"), "chddM") for row in selected),
