...
**Срез реализации: 10 сентября 2026, после полного аудита Y.** Трек 1: `5f167bb`;
Трек 2 и обучение Google TimesFM: `d917906`.
...
**Следующий эксперимент Z:** восемь новых физических сценариев завершены;
дообучение `71bae51` выполняется на A100. За ним поставлены независимая
проверка `1357ace` и новый цикл Qwen → OPM → ЧДД `f91143e`.
Это очередь эксперимента; таблица ниже сохраняет последний завершённый результат.
[Протокол следующего полного цикла](../deliverables/forecast_validation_20260910/z-transition-policy-pressure-wait-protocol.json).
...
последний прямой прогноз даёт **13,014–17,075% WAPE нефти**,
а первый месяц двух резких BHP-воздействий — **64,97–71,50%**.
Последние полностью проверенные веса: 40 эпох, выбрана эпоха 39 по прежней валидации; обучены
...
| Z: независимые новые локальные BHP-сценарии | [Новые метрики и абляция](../deliverables/forecast_validation_20260910/README.md) | Раннее кодирование и полное дообучение: нефть WAPE 13,014–17,075%, жидкость 11,484–20,838%, давление RMSE 19,526–23,748 бар; автономный допуск не закрыт |
