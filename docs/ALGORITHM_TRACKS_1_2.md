...
29 профильных проверок прошли на A100. Запуск `c8590c0` завершён: 11
прогнозов, три согласованных агентных предложения, фиксация варианта 10
и один финальный OPM. ЧДД **12 887,475236 против 11 873,620637 млн руб.,
+8,538715% — ниже требуемых 15%**. График после OPM не перевыбирался.
[Статус и доказательства](../deliverables/final_only_20260910/README.md).

**Исторический срез до нового обучения:** веса финального поиска — 40 эпох, WAPE месячной нефти 17,665–17,901%
...
повторяемости, а не ошибки прогноза. На финальном выбранном графике
WAPE месячной нефти 58,132551%, BHP MAE 50,288452 бар. Первые четыре эпохи
дообучения `cf925c2` прошли; независимые данные и финальный результат
в этот заранее закреплённый набор не включены.
[Протоколы](../deliverables/economic_targets_20260910/README.md).

**Уточнение рабочего режима:** трек 1 использует OPM итеративно. В треке 2
сначала оптимизируется график по прогнозу TimesFM, затем выбранный график
фиксируется и проверяется OPM в конце. Отдельные OPM-наборы для обучения и
независимой проверки относятся к исследованию. ТЗ §§1.2, 3.2.2 требует
управления на основе суррогата без постоянных запусков полной ГДМ; режим
одной финальной проверки — уточнение пользователя. Текущий выбор максимума
по банку ранее рассчитанных физических вариантов не доказывает этот режим.
Новый рабочий контур выполнен, но порог ЧДД и малая ошибка не достигнуты.

**Ограничения старого поиска:** аудит выявил нарушения собственных LRAT/WRAT/BHP,
не охваченные прежним `forecast_eligible`; новый guard добавлен в `aea630a`. Физический финальный
вариант соблюдает LRAT/WRAT, но имеет два исключения BHP. Их причина пока
не доказана; полная допустимость не подтверждена.
[Подробный аудит](../deliverables/final_only_20260910/bounded-forecast-diagnostics.json).
...
**Новый поиск переводов, 10 сентября:** Y (`8e3b2ae`) завершил месячный поиск; постаудит SUMMARY открыт.
...
**Срез реализации: 10 сентября 2026, после нового независимого теста Z.** Трек 1: `5f167bb`;
последний завершённый финансовый цикл Z: `464169a`; обучение экономических выходов: `5392d27`.
**Исходный пробел поиска:** в прежних завершённых запусках обоих треков новые переводы
добывающих скважин под закачку были отключены конфигурацией. Поддержка в коде
и расходы исходных переводов не доказывают оптимизацию новых переводов.
...
**Существенная оговорка после чтения чата:** базовый OPM-расчёт Z выдал
`PINCHREG: keyword not supported`. Эквивалентность физике tNavigator пока
не доказана; финансовые числа ниже подтверждены внутри OPM.
Коридор компенсации и зональный водный баланс требуют дополнительных проверок.
[Разбор комментариев и проверенные свидетельства](ORGANIZER_CHAT_CLARIFICATIONS_20260910.md).

**Прежний исследовательский эксперимент Z:** дообучение и независимая проверка `1357ace`
завершены. После отказа прежнего предложения из-за ранних открытий скважин
запущен новый полный цикл `f47e042`: даты первого WCON и сборка расписания
с исходными пределами BHP проверяются перед прогнозом. Новое предложение
Qwen и полный OPM/ЧДД завершены: **14 135,013999 млн руб., +19,045525%**.
Численное сравнение проверено; повторный аудит имеет одобрение критика,
но не всех ролей, поэтому строгий этап согласования не пройден. [Протокол](../deliverables/guarded_policy_20260910/README.md).

Отдельно завершены восемь новых физических сценариев и калибровка/проверка
интервалов фиксированной модели (`0d9e958`); полного допуска не получено. Прежние группы использовались
для обучения, поэтому их интервалы нельзя выдавать за независимые.
[Разделение и ограничения метода](../deliverables/fresh_uncertainty_20260910/README.md).

В отдельном [исследовании PINCH](../deliverables/pinch_compatibility_20260910/README.md)
подтверждён сбой OPM на схлопнутых ячейках; upstream-исправление собрано и
проверено, полный повтор пары завершён (`b11c5d9` / `669b17b`):
база 7 750,444366 → график 8 118,940837 млн руб., **+4,754521%**.
Цель 15% в эксперименте не достигнута. Основной движок сохраняется;
эквивалентность преобразования tNavigator не доказана. Таблицы ниже содержат
результаты прежнего OPM. Обученный TimesFM относится к прежней физике.
[Протокол нового полного цикла](../deliverables/forecast_validation_20260910/z-transition-policy-launch.json).

**Разделение по ТЗ (§§ 1.1–1.2, 3.2.1–3.2.2):** трек 1 принимает решения
по гидродинамическим расчётам, трек 2 использует обучаемый суррогат. TimesFM
удалён из рабочего контроллера трека 1; старый параметр `forecast` отклоняется.
Прежние опыты Y с TimesFM сохранены как история исследований. Их +54,915631%
не являются результатом нового цикла без суррогата. Новый цикл завершён:
49 скважин × 23 месяца, 1127 команд, 23 принятых заключения;
`surrogate_used=false`, ЧДД **+54,885597%**.
...
|---|---|---|
| Принцип | Qwen → полный физический расчёт вариантов → исполнение месяца → новое состояние → новое решение | Обученный TimesFM и Qwen → прогноз полных политик → полный физический пересчёт |
...
| Расчёт отклика | **OPM Flow**, полный остаток периода для каждого варианта; суррогат отсутствует | **Google TimesFM 3.0**: обученный прогноз 224 месяцев с геологией и плановыми ковариатами |
...
| Проверенный результат полного периода | 1 030,940234 → 1 596,777934 млн руб.; **+54,885597%**, без суррогата | 11 873,620637 → 14 135,013999 млн руб.; **+19,045525%** в последнем TimesFM/Qwen-цикле; сохранён лучший физический график **+19,123503%** |
...
а первый месяц новых BHP-воздействий — **16,598–54,950%**.
Последнее дообучение: ещё 40 эпох, выбрана эпоха 36 только по валидации; обучены
все 330,84 млн параметров, геология/идентичность скважин введены перед первым
и последним блоками внимания. Нулевой исторический масштаб заменяется
масштабом только обучающих данных. Будущая физическая опорная траектория
в этом прогнозе не используется;
[новые метрики и границы](../deliverables/forecast_validation_20260910/README.md).
Ни один прогноз не разрешает управление без физической проверки.
[Текущая приёмочная матрица и результаты](BOTH_TRACKS_ACCEPTANCE_20260909.md).
Загрузка обученных месячных весов Y ранее проверялась в отдельном эксперименте.
Она удалена из рабочего контроллера трека 1; [квитанция прежнего опыта](../deliverables/forecast_validation_20260910/y-monthly-integration-preview.json)
сохраняет исходную версию и не описывает текущую архитектуру.
Исторические бенчмарки ниже помечены отдельно; они сохраняют свои исходные
веса, разбиения, периоды и численные режимы.
...
flowchart TB
    ...
    Z --> S["TimesFM 3.0<br/>закреплённые обученные веса + геология"]
...
**Граница схемы.** Исправленная ветвь Y завершена без TimesFM: **+54,885597%**.
Полный аудит проверил 23 месячных решения, 1127 команд и 364 файла
происхождения; `surrogate_used=false`.
В Z последний график после обученного прогноза дал **+19,045525%**; он уступил
ранее найденному физическим поиском +19,123503%, поэтому лучший сохранён.
Эти результаты подтверждают ЧДД конкретных графиков. Пригодность суррогата
для самостоятельного выбора новых управлений проверяется отдельно.
...
| **TimesFM 3.0, только трек 2** | История рядов, плановые ковариаты, геология, закреплённые веса соответствующего кейса | Совместно прогнозирует будущие дебиты и давление | Массивы прогнозов; квантильный выход там, где запрошен | Поддерживает гипотезы управления; экономия OPM-вызовов требует отдельного замера |
...
<a id="training"></a>
## 6. TimesFM 3.0 в треке 2: данные и обучение

**TimesFM уже предобучен Google.** Основной поиск загружает закреплённую
ревизию Google, затем отдельные обученные веса своего кейса. Y и Z не делят
геологию или обученные контрольные точки. Выполнены дообучения выходного
слоя и последнего блока со статическими признаками; полное дообучение всей
сети Z проходит отдельную проверку и пока не заменяет принятые веса.
...
| Дообучение TimesFM | Выходной слой, статическая поправка и последний блок обучаются по целым OPM-сценариям; полный backbone — отдельный эксперимент | Адаптировать Google к геологии и управляющему отклику; выбирать эпоху только по валидации |
...
### 6.4. Исторический эксперимент: дообучение выходного блока TimesFM 3.0

Ниже сохранён первый опыт с 60 эпохами. Он объясняет процедуру, но не
описывает последние веса: последующие сценарные проверки и дообучения
обоих треков приведены в [приёмочной матрице](BOTH_TRACKS_ACCEPTANCE_20260909.md).
...
внутри TimesFM. Текущий адаптер передаёт адресную закачку, вычисленную
`WellConnectivity` для каждого месяца и всех связей, отдельным плановым
рядом каждой скважины. Обученные блоки получают идентичность скважины и
22 статических признака её геологии. Приор построен из OPM TRANX/Y/Z;
для Z включены 3865 NNC. THPRES Y остаётся в полной физической модели:
статический граф не воспроизводит нелинейное открытие барьера по давлению.
[Источники, ограничения и проверки геологии](RESERVOIR_AGENTS_GEOLOGY_20260909.md).
...
3. Запускает TimesFM на полном горизонте запроса — для проверенного Z это 224 месяца — с одной и той же доступной до начала историей.
...
В этом историческом опыте средняя WAPE нефти за 224 месяца — **94,72%
...
### 8.1. Исторические измерения исходных весов Google
...
| Покрытие интервалов для новых политик | Оценивается по отложенным целым сценариям в `evaluate_timesfm_scenarios.py`; последние тесты не подтверждают надёжность полной траектории давления |
| Область применимости для BHP/переводов | Выполнены независимые OPM-тесты общих режимов и BHP; точности полного Z недостаточно, перенос на произвольные переводы не сертифицирован |
...
действия: тогда TimesFM получает 515 собственных управляющих каналов и,
при включённой геологии, ещё 103 канала адресной закачки. Без BHP в исходном
наборе запрос изменения давления отклоняется. Фактически выбранные изменения
определяются сохранённым запросом, а не возможностями JSON-схемы.

$$
S_B=\frac{\hat M_o(P_o-D_o-C_o)-\hat M_l C_l-V^{plan}_{inj} C_{inj}}{10^6}.
$$

Это оценка маржи заданного горизонта без всех событийных расходов и налогов.
Даже при 224 месяцах она не равна официальному ЧДД. Полная экономика
начисляется отдельно по физическому отклику и исходным нормативам.

**C: происхождение физического поиска.** После исходного сравнения агент
...
ЧДД с включённой базой. Первый этап сохранён в
[`selection-final-reviewed.json`](../deliverables/track2_model_z/full_horizon_search_20260909/selection-final-reviewed.json).
Последующий численный физический поиск дал +19,123503%; кандидат с ранними
весами — +18,686148%; новый TimesFM/Qwen-кандидат — +19,045525%.
[Последнее численное сравнение](../deliverables/guarded_policy_20260910/numeric-selection.json)
сохраняет лучший график. Повторный аудит не получил согласия всех четырёх ролей.
...
С 10 сентября добавлены `max_monthly_water_deficit_m3`,
`min_monthly_voidage_replacement`, `max_monthly_voidage_replacement`.
Они проверяются по **приращениям накопленных объёмов за месяц**, после OPM:
водный дефицит = max(0, ΣΔWWIT − ΣΔWWPT), в поверхностных м³;
компенсация = ΣΔWVIT / ΣΔWVPT, в пластовых объёмах.
При нулевом отборе положительная закачка нарушает верхний предел; полностью
неработающая группа проходит проверку. Суррогат не подтверждает эти ограничения.
Группа скважин и пределы задаются явно в кейсе. Отсутствие нужных векторов
или начальной месячной точки вызывает отказ. Прежние экспорты без WVPT/WVIT
принимаются, если такие ограничения не запрошены.
Это проверка месячного нетто-баланса выбранного фонда: она не моделирует
запасы воды, потери очистки, транспортную сеть или перетоки между геологическими
зонами. Для этого остаются отдельные требования C04/C05.
...
сценарии сравниваются внутри одного режима; финальные финансовые результаты выше
сопоставлены с базой в одинаковом режиме MPI=16. Повторная последовательная
проверка именно новых выбранных графиков не заявляется выполненной. [Замеры, конфигурация и ограничения](A100_PERFORMANCE_20260909.md).
...
|---|---|---|
| Y: Qwen/OPM без суррогата, 49 скважин, 23 месяца | [Полный аудит](../deliverables/forecast_validation_20260910/y-physical-only-full-audit.json) | 1 030,940234 → 1 596,777934 млн руб.; **+54,885597%**; `surrogate_used=false` |
| Z: обученный TimesFM/Qwen, 103 скважины, 224 месяца | [Сравнение и раздельные решения ролей](../deliverables/guarded_policy_20260910/README.md) | Последний цикл **+19,045525%**; сохранён лучший физический график **+19,123503%** |
| Z: происхождение лучшего графика | Исходные пути, SHA-256 и отдельные пары в том же сравнении | Лучший найден физическим поиском; вклад нового обученного прогноза нельзя приписывать старому графику |
| Z: независимые новые BHP-сценарии | [Парное сравнение весов](../deliverables/forecast_validation_20260910/README.md) | Расширенное обучение: нефть WAPE 9,730–11,020%, жидкость 5,583–8,008%, WBP9 RMSE 15,210–15,498 бар; все метрики улучшены на каждом новом тесте; автономный допуск не закрыт |
| Исторический опыт Y: отдельное обучение месячного прогноза | [Свежие независимые режимы v3](../deliverables/forecast_validation_20260910/y-intervention-independent-v3.json) | Новое обучение: нефть 1,36–1,91%, первый месяц 6,50–13,86%; интеграционный опыт остановлен; эти веса исключены из рабочего трека 1 |
...
к текущей поставке Трека 2. Исторические ссылки Трека 1 ниже объясняют
основной MPC; добавленный прогнозный модуль закреплён отдельно.
...
| Трек 1: агент и хвост | [run_track1_mpc.py @ 5f167bb](https://github.com/DarkPrinceWarrior/TimesOil/blob/5f167bb/scripts/run_track1_mpc.py): `execute`, `_propose_controls`, `_continuation_tail`, `_resume_steps` |
...
**Связанные материалы:** [передача 10 сентября](HANDOFF_20260910.md)
