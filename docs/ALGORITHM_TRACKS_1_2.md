...
**Срез реализации: 9 сентября 2026.** Трек 1: `aa2daf7`; Трек 2: `739200b`.
...
Полный перенос всех управляющих входов и доказательств в TimesFM ещё не завершён.
...
`qwen-3.8-27b` через Cerebras API. Это разные инструкции и права инструментов
...
**TimesFM уже предобучен Google.** Текущие скрипты TimesOil загружают
закреплённые веса и выполняют прогноз; они не обучают эти веса на десяти
наших OPM-сценариях. Обучение пяти ансамблевых участников относится к
другой модели — CRM + LightGBM в [приложении](#comparison).
...
| Дообучение TimesFM | **В текущих скриптах не выполняется** | Нельзя приписывать модели несуществующую адаптацию весов |
...
Для каждой скважины код строит четыре плановых канала:
...
Они образуют **412 каналов** для 103 скважин. Передаются на всю длину
`context + horizon`: прошлые управления и известные плановые будущие.

```text
past_future_covariates: [412, context + horizon]
forecast:              [309, horizon]
после reshape:         [horizon, 103, 3]
```

`forecast_inputs` также умеет сформировать пятый канал — суммарную полевую
закачку, повторённую для каждой скважины. Основной совместный вызов в
`propose_track2_policies.py` и `benchmark_horizons.py` использует первые
четыре: `cov[:, :4]`. При этом ему доступны WRAT всех скважин одновременно.
Нельзя утверждать, что передаются 515 каналов, если в вызове реально 412.
...
1. Применяет множители к полному базовому графику; учитывает список изменений отдельных скважин и закрытия.
...
| Будущий `bhp_limit` | **В основном `propose_policy` не подключён**; запрос с BHP-изменениями отклоняется | Нужен отдельный адаптер и проверка отклика |
| Фактический BHP будущего | Не известен до физического расчёта | OPM WBHP и проверки ограничений |

Добавление `bhp_limit` к входам CRM + LightGBM не исправляет этот пробел
TimesFM. Нужна проверка именно Google-модели с этим управляющим каналом;
её нельзя заменить переименованием поля или отчёта другой модели.
...
множители, список закрываемых скважин. Инструмент строит полный график,
...
BHP-варианты отклоняются: данный маршрут не имеет BHP-ковариаты.
...
| BHP-эксперимент CRM + LightGBM | [Скрипт сравнения](../scripts/benchmark_bhp_surrogate.py) | Отдельная модель; это не обучение TimesFM и не закрытие его BHP-входа |
...
#### A.1.1. Подготавливаемый BHP-эксперимент
...
| Статус | Серия на A100; итог фиксируется только после появления проверенного `report.json` |
