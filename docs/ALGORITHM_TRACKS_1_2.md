...
**Срез реализации: 10 сентября 2026, 04:58 UTC.** Трек 1: `66c09fb`;
Трек 2 и обучение Google TimesFM: `024ea4f`.
...
|---|---|---|
| Принцип | TimesFM и Qwen → решение → исполнение месяца → новое состояние → новое решение | Обученный TimesFM и Qwen → прогноз полных политик → полный физический пересчёт |
...
| Основной прогнозный путь | **Google TimesFM 3.0**: обученный прогноз полного остатка; отдельный предобученный месячный монитор | **Google TimesFM 3.0**: обученный прогноз 224 месяцев с геологией и плановыми ковариатами |
...
| Проверенный результат полного периода | 1 030,940234 → 1 597,087572 млн руб.; **+54,915631%**, все 23 месяца приняты | 11 873,620637 → 14 128,268207 млн руб.; **+18,988712%** в новом TimesFM/Qwen-цикле; сохранён лучший физический график **+19,123503%** |
...
**ЧДД ≥15% подтверждён отдельно для обоих учебных кейсов.** Точность нового
автономного прогноза и UQ/OOD остаются отдельной незавершённой проверкой:
прямой прогноз Z после полного дообучения даёт 17,75–18,36% WAPE нефти.
Локальный прогноз с физическим эталоном точнее, но абляция без Google ещё лучше;
...
Новая загрузка отдельных обученных месячных весов Y проверена на A100 вместе
с полным прогнозом; она не меняет происхождение ранее завершённого 23-месячного
результата. Поля `forecast.monthly_head` и `forecast.monthly_report` требуют
своих SHA-256, завершённого отчёта обучения на один месяц и совпадающей геологии.
Историческая калибровка повторяется для загруженных весов. Без этих полей
сохраняется исходный месячный монитор. [Проверка интеграции](../deliverables/forecast_validation_20260910/y-monthly-integration-preview.json).
...
flowchart TB
    ...
    A --> F1["TimesFM: месяц + полный остаток<br/>геология · плановые управления · история"]
    F1 --> P["Валидатор + продолжение до конца<br/>текущая политика и предложение"]
...
**Граница схемы.** Обе ветви выполнены с реальными вызовами TimesFM и Qwen.
В Y полный аудит проверил все 23 месячных решения и 364 файла происхождения.
В Z новый график после обученного прогноза дал +18,988712%; он немного уступил
...
| **TimesFM 3.0** | История рядов, плановые ковариаты, геология, закреплённые веса соответствующего кейса | Совместно прогнозирует будущие дебиты и давление | Массивы прогнозов; квантильный выход там, где запрошен | Поддерживает гипотезы управления; экономия OPM-вызовов требует отдельного замера |
...
`scripts/timesfm_planning.py` подключён к агентному месячному запуску.
Каждая гипотеза получает прогноз для всех 49 скважин: отдельный обученный
выходной слой/последний блок для полного остатка и исходные веса Google для
одного месяца с собственной исторической калибровкой. Агент получает геологию,
плановые воздействия, прогнозы, ошибки уже исполненных месяцев и OOD-диагностику.
После принятия шага в историю добавляется ровно одно наблюдение из проверенного
OPM-экспорта; будущий хвост физического планирования в историю не поступает.

Окончательный выбор по-прежнему требует OPM и официального ЧДД полного остатка.
Отдельный интерфейс `MonthlyMPC.prescreen` не означает, что этот прогноз
автоматически отбрасывает кандидатов или заменяет физическое сравнение.
Историческая месячная калибровка не подтверждает интервалы для нового режима
или полного остатка. Полный действующий цикл завершён: +54,915631%.
...
Последующий численный физический поиск дал +19,123503%; новый обученный
TimesFM/Qwen-кандидат — +18,988712%. Итоговое сравнение сохраняет оба
происхождения и лучший график:
[`timesfm-trained-z-policy-selection.json`](../deliverables/control_coverage_20260909/timesfm-trained-z-policy-selection.json).
...
|---|---|---|
| Y: TimesFM/Qwen, 23 месяца, 49 скважин, 1 127 действий; 23 одобренные месячные проверки | [Полный месячный аудит](../deliverables/control_coverage_20260909/timesfm-y-complete-monthly-audit.json) | 1 030,940234 → 1 597,087572 млн руб.; **+54,915631%** |
| Z: обученный TimesFM/Qwen, 103 скважины, 224 месяца | [Полное сравнение и агентский аудит](../deliverables/control_coverage_20260909/timesfm-trained-z-policy-selection.json) | Новый цикл **+18,988712%**; сохранён лучший физический график **+19,123503%** |
...
| Z: независимые новые локальные BHP-сценарии | [Новые метрики и абляция](../deliverables/forecast_validation_20260910/README.md) | Полное дообучение: прямой прогноз нефти 17,75–18,36%; физическая абляция без Google лучше локальной поправки с Google; автономный допуск не закрыт |
| Y: отдельное обучение месячного прогноза | [Свежие независимые режимы v3](../deliverables/forecast_validation_20260910/y-intervention-independent-v3.json) | Новое обучение: нефть 1,36–1,91%, первый месяц 6,50–13,86%; новая интеграция проверена, полный контрольный цикл выполняется |
...
| TimesFM в месячном Y-контуре | [timesfm_planning.py @ ca8e376](https://github.com/DarkPrinceWarrior/TimesOil/blob/ca8e376/scripts/timesfm_planning.py): `TimesFMPlanning.predict`, `observe`, `committed_model_state` |
