...
**Текущий эксперимент Z:** восемь физических сценариев, 40 эпох дообучения
и независимая проверка `1357ace` завершены. Новый цикл Qwen → OPM → ЧДД
`ba112c1` остановился до OPM из-за неверного Python-окружения. Повтор `87f1a96`
с рабочим Python отклонён из-за открытия скважин до первого исходного WCON.
Теперь этот запрет проверяется до прогноза, даты доступности передаются агенту.
Предложение сохранено, расчёт сейчас не идёт; таблица содержит последний завершённый результат.
